...
const approveRetirement = async (req, res) => {
  ...
  try {
    ...
    if (!reqRows.length) {
      ...
      return res.status(404).json({ error: 'Retirement request not found' });
    }
    ...
    if (rr.status !== 'pending') {
      ...
      return res.status(409).json({
        error: `Request already ${rr.status}`,
        status: rr.status,
      });
    }
    ...
    if (dupRows.length) {
      await client.query('ROLLBACK');
      return res.status(409).json({
        error: 'This serial number has already been retired',
        certId: dupRows[0].certificate_id,
      });
    }
    ...
    console.error('[approveRetirement]', err.message);
...
const rejectRetirement = async (req, res) => {
  ...
  try {
    ...
    if (rows[0].status !== 'pending') {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: `Request already ${rows[0].status}` });
    }
    ...
    await client.query('ROLLBACK');
    console.error('[rejectRetirement]', err.message);
