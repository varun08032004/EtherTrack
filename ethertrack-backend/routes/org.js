...
router.post('/create', authenticate, orgActionLimiter, async (req, res) => {
...
router.post('/:orgId/invite', authenticate, requireRole('owner', 'admin'), orgActionLimiter, async (req, res) => {
...
router.post('/accept-invite', authenticate, orgActionLimiter, async (req, res) => {
...
router.patch('/:orgId/members/:userId/role', authenticate, requireRole('owner'), orgActionLimiter, async (req, res) => {
...
router.delete('/:orgId/members/:userId', authenticate, requireRole('owner', 'admin'), orgActionLimiter, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT team_role FROM org_members WHERE org_id=$1 AND user_id=$2`,
      [req.params.orgId, req.params.userId]
...
router.post('/:orgId/verifiers/request', authenticate, requireRole('owner', 'admin'), orgActionLimiter, async (req, res) => {
...
router.post('/:orgId/retirement-queue', authenticate, requireRole('owner', 'admin', 'manager'), orgActionLimiter, async (req, res) => {
  const {
...
router.post('/:orgId/retirement-queue/:itemId/reject', authenticate, requireRole('owner', 'admin'), orgActionLimiter, async (req, res) => {
  const { orgId, itemId } = req.params;
...
router.post('/plan/create-order', authenticate, orgActionLimiter, async (req, res) => {
...
router.post('/plan/select', authenticate, orgActionLimiter, async (req, res) => {
  const {
...
router.post('/:orgId/verifiers', authenticate, requireRole('owner', 'admin'), orgActionLimiter, async (req, res) => {
  const { verifierName, verifierCode, contactEmail, notes } = req.body;
  if (!verifierName) return res.status(400).json({ error: 'Verifier name required' });
  try {
    const { rows } = await query(
