// routes/org.js — EtherTrack Organisation & Team Management
...
router.post('/create', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (existing.length) {
      return res.status(409).json({ error: 'You already own an organisation. Contact support to create another.' });
    }
    ...
    // Update user's org_id and team_role
    await query(
    ...
    console.error('Org create error:', e.message);
    res.status(500).json({ error: 'Failed to create organisation', detail: e.message });
  }
});

// ── GET /api/org/me ──────────────────────────────────────────────
// Get current user's organisation + their role
router.get('/me', authenticate, async (req, res) => {
  try {
    ...
    const org      = rows[0];
    const teamRole = org.team_role;
    const perms    = getPermissions(teamRole);

    res.json({ org, teamRole, permissions: perms });
  } catch (e) {
  ...
  try {
    const { rows } = await query(
      `SELECT om.id, om.team_role, om.status, om.invited_at, om.accepted_at,
       ...
       LEFT JOIN org_invites inv ON inv.org_id = om.org_id AND inv.email = u.email
       WHERE om.org_id = $1
  ...
  try {
    ...
    try {
      await sendEmail({
        ...
        html:    `
          <div style="font-family:monospace;background:#040706;color:#f0fdf4;padding:32px;border-radius:12px;max-width:520px;">
            <div style="color:#22c55e;font-size:18px;font-weight:700;margin-bottom:12px;">EtherTrack 🌿</div>
            <p style="color:#86efac88;margin-bottom:20px;">
              You've been invited to join <strong style="color:#f0fdf4">${org[0]?.name}</strong> as a
              <strong style="color:#22c55e">${teamRole}</strong>.
            </p>
            <a href="${inviteUrl}" style="display:inline-block;padding:12px 24px;background:#14532d;color:#d1fae5;border-radius:8px;text-decoration:none;font-weight:700;letter-spacing:.08em;">
              ACCEPT INVITATION →
            </a>
            <p style="color:#86efac33;font-size:12px;margin-top:20px;">This invite expires in 7 days.</p>
          </div>`,
      });
    } catch (emailErr) {
...
// ── GET /api/org/invite-preview ──────────────────────────────────
// Public preview of invite details (org name, role) before accepting
// Used by JoinOrg.js to show what the user is joining
...
router.post('/accept-invite', authenticate, async (req, res) => {
  ...
  try {
    ...
    // Update user's org_id + team_role
    await query(
      `UPDATE users SET org_id=$1, team_role=$2 WHERE id=$3`,
      [inv.org_id, inv.team_role, req.user.id]
    );

    const { rows: org } = await query(`SELECT name FROM organisations WHERE id=$1`, [inv.org_id]);
...
router.patch('/:orgId/members/:userId/role', authenticate, requireRole('owner'), async (req, res) => {
  ...
  try {
    await query(
      `UPDATE org_members SET team_role=$1 WHERE org_id=$2 AND user_id=$3`,
      [teamRole, req.params.orgId, req.params.userId]
    );
    await query(
      `UPDATE users SET team_role=$1 WHERE id=$2`,
      [teamRole, req.params.userId]
    );
...
router.delete('/:orgId/members/:userId', authenticate, requireRole('owner','admin'), async (req, res) => {
  try {
    ...
    if (rows[0]?.team_role === 'owner')
      return res.status(403).json({ error: 'Cannot remove org owner' });

    await query(
      `UPDATE org_members SET status='revoked' WHERE org_id=$1 AND user_id=$2`,
      [req.params.orgId, req.params.userId]
    );
    await query(
      `UPDATE users SET org_id=NULL, team_role='viewer' WHERE id=$1`,
      [req.params.userId]
    );
...
// ── POST /api/org/:orgId/verifiers/request ───────────────────────
// Provision: user requests a verifier connection
router.post('/:orgId/verifiers/request', authenticate, requireRole('owner','admin'), async (req, res) => {
  ...
  try {
    ...
    // TODO: When verifier API is ready, send connection request here
    // For now just log the request
    console.log(`🔍 Verifier connection requested: ${verifierName} for org ${req.params.orgId}`);

    res.json({
      message: `Request submitted for ${verifierName}. We will contact them on your behalf.`,
      verifier: rows[0],
    });
  } catch (e) {
    res.status(500).json({ error: 'Failed to submit verifier request' });
  }
});

// ── GET /api/org/:orgId/audit-log ────────────────────────────────
...
// ── GET /api/org/:orgId/permissions ─────────────────────────────
// Returns current user's permissions in this org
