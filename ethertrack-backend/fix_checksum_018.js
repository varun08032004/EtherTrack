...
async function fixChecksum() {
  try {
    ...
    const migrationPath = path.join(__dirname, 'db', 'migrations', '018_asset_eligibility_rules.sql');
    ...
    console.log('Checksum updated for migration 018');
    
    // Verify
    const { rows } = await pool.query(`SELECT version, checksum FROM schema_migrations WHERE version = '018'`);
