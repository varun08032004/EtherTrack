 ...
 *
 * Individual dashboard card components.
 * Each is memo-wrapped and receives only its own slice of state.
 * Zero cross-card re-renders.
 */
...
export const PortfolioValueCard = memo(function PortfolioValueCard({
  ...
  return (
    <div className={`${s.card} ${s.c3}`} role="region" aria-label="Portfolio value">
      ...
      {pnl !== null && !loading && (
        <div
          className={pnl >= 0 ? `${s.cchg} ${s.pnlPos}` : `${s.cchg} ${s.pnlNeg}`}
...
export const ETHWalletCard = memo(function ETHWalletCard({
  ...
  // ethRate can be null until first fetch — never show a hardcoded fallback
  ...
  return (
    <div className={`${s.card} ${s.c8}`} role="region" aria-label="Live carbon credit market">
      <div className={s.clabel}>
        ...
        <span className={s.liveBadge} aria-hidden="true">
      ...
      {listings.length > 0 ? (
        <>
          ...
          {listings.slice(0, 4).map((c, i) => {
            // Only show price if ethRate is available — never show ₹0 from stale rate
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 11, padding: '2px 6px', borderRadius: 3, background: `${reg}22`, color: reg, border: `1px solid ${reg}33` }}>
                  {c.standard}
                </span>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#22c55e' }}>
                  {priceINR !== null
                    ? `₹${priceINR.toLocaleString('en-IN')}`
                    : <span className={s.shimmer} style={{ display: 'inline-block', width: 60, height: 14 }} />}
                </span>
...
export const EmissionOffsetCard = memo(function EmissionOffsetCard({
  ...
  const { pct, creditsRetiredButNoEmissions } = calcEmissionOffset(totalEmitted, totalRetiredCount);
  const net = totalEmitted > 0 ? Math.max(0, totalEmitted - totalRetiredCount) : null;

  return (
    <div className={`${s.card} ${s.c4}`} role="region" aria-label="Your emission offset progress">
      <div className={s.clabel}>
        MY EMISSION OFFSET <span style={{ fontSize: 11, color: '#86efac33' }}>MY DATA</span>
      </div>
      {emissionsError ? (
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 0' }}>
            <EmissionArc pct={pct} creditsRetiredButNoEmissions={creditsRetiredButNoEmissions} />
            <div style={{ marginTop: -4, textAlign: 'center' }}>
              <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 30, fontWeight: 800, lineHeight: 1, color: creditsRetiredButNoEmissions ? '#4ade80' : '#22c55e' }}>
                {creditsRetiredButNoEmissions ? '✓' : `${pct}%`}
              </div>
              <div style={{ fontSize: 11, color: '#86efac55', marginTop: 3, letterSpacing: '.06em' }}>
                {creditsRetiredButNoEmissions
                  ...
                  : totalEmitted > 0 ? 'OF YOUR LOGGED EMISSIONS' : 'LOG EMISSIONS TO TRACK OFFSET'}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 10 }}>
            {[
              { l: 'MY EMITTED',     v: totalEmitted > 0    ? `${totalEmitted} tCO₂`             : '—', c: '#f87171' },
          ...
          <LastRefreshed ts={emissionsTs} />
        </>
      )}
      <button className={s.btnGhost} onClick={() => navigate('/emission-tracking')}>LOG EMISSIONS →</button>
    </div>
  );
});
      ...
      {breakdown.length > 0 ? breakdown.map(({ name, std, credits, color, pct }) => (
        <div key={name} style={{ marginBottom: 10 }}>
          ...
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`${name} ${pct}% of portfolio`}
          >
  ...
  return (
    <div className={`${s.card} ${s.c8}`} role="region" aria-label="Carbon market news">
      ...
      {newsItems.map((n, idx) => {
        ...
        const safeUrl = n.url; // already sanitized by sanitizeNewsItem
        return (
          <div
            key={n.id || idx}
            ...
            onClick={() => safeOpen(safeUrl)}
            onKeyDown={(e) => e.key === 'Enter' && safeOpen(safeUrl)}
          >
            <span className={s.newsTag} style={{ background: tc.bg, color: tc.c, border: `1px solid ${tc.border}` }}>
              {n.tag}
            </span>
            <div style={{ flex: 1 }}>
      ...
      <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {NEWS_SOURCE_LINKS.map(({ label, url }) => (
          <a key={label} href={url} target="_blank" rel="noopener noreferrer" className={s.newsSrcLink}>
            {label} ↗
          </a>
        ))}
      </div>
  ...
  return (
    <div className={`${s.card} ${s.c4}`} role="region" aria-label="Recent trades" aria-busy={tradesLoading}>
      <div className={s.clabel}>
        RECENT TRADES
        <button
          style={{ cursor: 'pointer', color: '#22c55e44', fontSize: 11, background: 'none', border: 'none', fontFamily: 'inherit', padding: 0 }}
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div
                  style={{ width: 34, height: 34, borderRadius: 6, background: isBuy ? '#0d2e1f' : '#450a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: isBuy ? '#22c55e' : '#f87171' }}
                  aria-hidden="true"
                >
