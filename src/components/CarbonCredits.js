...
import { v4 as uuidv4 } from 'uuid'; // ✅ FIX: idempotency keys
...
// ── ✅ FIX: buildPriceHistory no longer returns fake random data ──
// Returns [] when no real trades exist — chart simply won't render.
// Fake trend lines mislead buyers into thinking price has moved.
function buildPriceHistory(tradeHistory, tokenId) {
  const relevant = tradeHistory
    .filter(t => t.tokenId === tokenId)
    .slice(0, 30)
    .reverse();
  if (relevant.length < 2) return []; // ✅ no fake data
  return relevant.map(t => parseFloat(t.totalEth) / (t.amount || 1) * ETH_INR);
}

// ── ✅ FIX: Sub-components defined OUTSIDE parent ─────────────────
// Previously defined inside CarbonCredits — caused full remount on every
// parent state change. Now defined at module scope and receive props.
...
// ── WatchlistPanel — module-level component ───────────────────────
        ...
        return (
          <div key={l.listingId} className={`cc-wl-row${isSel?' sel':''}`} onClick={() => { setSelected(l); setAnalyticsToken(l); }}>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: 11, fontWeight: 500, color: isUp === null ? '#86efac88' : isUp ? '#22c55e' : '#f87171' }}>{fmt(price.toFixed(0))}</div>
              {/* ✅ FIX: Only render chart if real data exists */}
              {history.length >= 2 && <MiniChart data={history.slice(-10)} color={isUp ? '#22c55e' : '#f87171'} width={50} height={18}/>}
            </div>
          </div>
        );
      })}
...
// ── OrderBookPanel — module-level component ───────────────────────
function OrderBookPanel({ allAsks, allBids, selectedAsks, selectedBids, selected, liveETHINR, setSelected, setTab }) {
  ...
  const asks = obMode === 'all' ? allAsks : selectedAsks.map(l => ({
    price: l.adjPrice, priceInr: l.pricePerUnitINR || l.adjPrice * liveETHINR,
...
// ── CreditInfoCard — module-level component ───────────────────────
  ...
  return (
    <div className="cc-panel" style={{ marginBottom: 12 }}>
      ...
      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #0f2a1a' }}>
        ...
        {/* ✅ FIX: Only render chart with real data */}
        {history.length >= 2
          ? <MiniChart data={history} color="#22c55e" width={300} height={48}/>
          : <div style={{ height: 48, display:'flex', alignItems:'center', justifyContent:'center', color:'#86efac22', fontSize:9, letterSpacing:'.1em' }}>CHART AVAILABLE AFTER FIRST TRADE</div>
        }
      </div>
    </div>
  );
...
// ── OrderForm — module-level component ────────────────────────────
function OrderForm({
  ...
  liveETHINR, handlePlaceOrder, handlePlaceBid,
  navigate,
}) {
  const inrSufficient = inrBalance >= tradeNetInr && tradeNetInr > 0;

  // ✅ FIX: listingId selected → check qty doesn't exceed available
      ...
      <div className="cc-panel" style={{ marginBottom: 10 }}>
        ...
        {(orderMode === 'market' || orderMode === 'limit') && (
          <>
            ...
            {/* ✅ FIX: show max available + warn if over */}
            ...
            <button
              ...
              disabled={!isKYCVerified||txPending||qtyOverMax||(paymentMode==='eth'&&!walletAddress)||(paymentMode==='inr'&&(!inrSufficient||tradeNetInr<=0))}
...
export default function CarbonCredits() {
  ...
  // ✅ FIX: cancelBid confirm state
  const [cancelBidConfirm, setCancelBidConfirm] = useState(null);

  // ── ✅ FIX: CSS memoised — never recalculates unless theme changes ──
  const CSS = useMemo(() => `
  ...
  // ── ✅ FIX: useEffect deps use actual arrays, not .length ─────
  useEffect(() => {
    if (!listings.length) return;
    const h = {};
    listings.forEach(l => {
      // ✅ FIX: no fake data — pass real trades only
      h[l.tokenId] = buildPriceHistory(tradeHistory, l.tokenId);
    });
    setPriceHistories(h);
  }, [listings, tradeHistory]); // ✅ full arrays, not .length
  ...
  // ── ✅ FIX: MetaMask account change on page focus ─────────────
  useEffect(() => {
    const handleFocus = () => {
      if (window.ethereum) {
        window.ethereum.request({ method: 'eth_accounts' }).then(accounts => {
          if (accounts.length && accounts[0].toLowerCase() !== walletAddress?.toLowerCase()) {
            // Account changed while tab was in background — reload to re-init
  ...
  const filtered = listings
    ...
    // ✅ FIX: filter expired listings (API path missed this)
  ...
  const avgTradePrice = tradeHistory.length
    ...
    : 0; // ✅ FIX: no fake average from listing prices — show '—' if no trades
  ...
  const fetchBatchId = async (tokenId) => {
    try {
      const res = await fetch(`${API_URL}/api/portfolio/batch-by-token/${tokenId}`, {
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('et_access')}` },
      });
  ...
  const recordTrade = async ({ txHash, paymentMode, listing, qty, pricePerCreditINR }) => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/trades/record`, {
        ...
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('et_access')}` },
        body: JSON.stringify({
          batchId, listingId: listing.listingId, quantity: qty,
          paymentMode, txHash: txHash || null,
          pricePerCreditINR: pricePerCreditINR || listing.pricePerUnitINR || Math.round(listing.adjPrice * liveETHINR),
          idempotencyKey: txHash, // ✅ FIX: use txHash as idempotency key for ETH trades
        }),
      });
      const data = await res.json();
      if (!data.success) {
        // ✅ FIX: ETH DB miss is no longer silent — log to console with tx hash
        console.error('⚠️ ETH trade DB record failed — blockchain listener will retry:', data.error, '| txHash:', txHash);
      }
      return data;
    } catch (e) {
      console.error('⚠️ recordTrade fetch failed:', e.message, '| txHash:', txHash);
      return null;
    }
  };
  ...
  // ── ✅ FIX: handleConfirmBuy — INR path now single /record call ──
  // Removed the two-call (deduct → record) pattern entirely.
  // /api/trades/record now handles deduct + settle atomically on server.
  const handleConfirmBuy = async () => {
    const o = confirmModal;
    setConfirmModal(null);
    setTxPending(true);

    // ✅ FIX: generate idempotency key once per trade attempt
    ...
    try {
      if (o.paymentMode === 'inr') {
        ...
        // ✅ Single atomic call — deducts INR + credits seller + records trade
        const settleRes = await fetch(`${API_URL}/api/trades/record`, {
          ...
          body: JSON.stringify({
            ...
            idempotencyKey,    // ✅ prevents double-settlement on retry
          }),
        });
        ...
        if (!tradeData.success) {
          throw Object.assign(new Error(tradeData.error || 'Trade settlement failed'), {
      ...
      // ✅ FIX: INR path no longer needs refund here — server-side atomicity
      // means if /record fails, nothing was deducted.
      if (e.code === 4001) {
  ...
  // ✅ FIX: handleCancelBid now requires confirmation modal
  const handleCancelBidRequest = (orderId, ethEscrowed) => {
    setCancelBidConfirm({ orderId, ethEscrowed });
  };

  const handleCancelBidConfirmed = async () => {
    const { orderId } = cancelBidConfirm;
    setCancelBidConfirm(null);
    try {
  ...
  return (
    <>
        ...
        <div className="cc-wrap">
                ...
                : listings.map(l => {
                    ...
                    return (
                      <div key={l.listingId} className={`cc-tick${selected?.listingId===l.listingId?' sel':''}`} onClick={() => { setSelected(l); setTab('trade'); }}>
                        ...
                        <div className="cc-tick-price" style={{ color: isUp === null ? '#86efac88' : isUp ? '#22c55e' : '#f87171' }}>{fmt(price.toFixed(0))}</div>
          ...
          <div className="cc-stats">
            {[
              ...
              { label:'AVG TRADE PRICE',   val:avgTradePrice?fmt(avgTradePrice.toFixed(0)):'—',              sub: avgTradePrice ? 'per tonne CO₂' : 'no trades yet' },
          ...
          {tab === 'market' && (
            <div className="cc-market-layout">
              <WatchlistPanel
                ...
                setAnalyticsToken={setAnalyticsToken}
                watchlist={watchlist}
                    ...
                    : filtered.map(l => {
                        ...
                        return (
                          <div key={l.listingId} className={`cc-tbl-row${isSel?' sel':''}`} onClick={() => { setSelected(l); setAnalyticsToken(l); setTab('trade'); }}>
                            ...
                            <div>
                              <div style={{ fontSize:13, fontWeight:500, color: isUp === null ? '#86efac88' : isUp ? '#22c55e' : '#f87171' }}>{fmt(price.toFixed(0))}</div>
                              <div style={{ fontSize:9, color:'#86efac44' }}>{l.amount} avail</div>
                            </div>
                            <span style={{ fontSize:10, color:'#86efac66' }}>{l.vintageYear}</span>
                            {/* ✅ FIX: no fake chart — show dash if no data */}
                            {history.length >= 2
                ...
                <div className="cc-chart-wrap">
                  ...
                  {analyticsHistory.length >= 2
                    ...
                    : <div style={{ height:120, display:'flex', alignItems:'center', justifyContent:'center', color:'#86efac22', fontSize:10, letterSpacing:'.1em' }}>
                        CHART AVAILABLE AFTER FIRST TRADE ON THIS CREDIT
                      </div>
                  }
                </div>
              </div>
                ...
                : <>
                    ...
                    {myOpenBids.map(o => (
                      <div key={o.orderId} className="cc-bids-row">
                        ...
                        {/* ✅ FIX: Cancel requires confirm — no accidental ETH loss */}
                        <button
      ...
      {/* ✅ FIX: Cancel Bid Confirm Modal — prevents accidental ETH loss */}
      {cancelBidConfirm && (
          ...
          <div className="cc-modal" style={{ maxWidth:360 }}>
            ...
            <div className="cc-modal-b">
              <div style={{ padding:'12px', background:'#1a0707', border:'1px solid #f8717122', borderRadius:8, marginBottom:12, fontSize:11, color:'#f8717199', lineHeight:1.7 }}>
                This will cancel your bid and return <strong style={{ color:'#f87171' }}>{cancelBidConfirm.ethEscrowed?.toFixed(6)} ETH</strong> from escrow to your wallet.<br/>
                <span style={{ fontSize:9, color:'#f8717155' }}>Gas fees apply. This action cannot be undone.</span>
              </div>
            </div>
            <div className="cc-modal-f">
        ...
        <div className="cc-overlay" onClick={e => e.target===e.currentTarget && setAmmModal(null)}>
          <div className="cc-modal">
            ...
            <div className="cc-modal-b">
              ...
              {/* ✅ FIX: Real slippage calculation + price impact warning */}
              {ammQty && ammModal.pool && (() => {
                ...
                if (ammDir === 'buy' && pool.ethReserve > 0 && pool.creditReserve > 0) {
                  const inputWithFee = inputAmt * 0.997;
                  outputAmt = (inputWithFee * pool.creditReserve) / (pool.ethReserve + inputWithFee);
                  priceImpactPct = (inputAmt / pool.ethReserve) * 100;
                } else if (ammDir === 'sell' && pool.creditReserve > 0 && pool.ethReserve > 0) {
                  const inputWithFee = inputAmt * 0.997;
                  outputAmt = (inputWithFee * pool.ethReserve) / (pool.creditReserve + inputWithFee);
                  priceImpactPct = (inputAmt / pool.creditReserve) * 100;
                }

                // ✅ FIX: minOut = 2% slippage tolerance (not 0)
                ...
                return (
                  <div style={{ background:'#040706', borderRadius:6, padding:'9px 11px', marginBottom:10 }}>
                    ...
                    <div className="cc-fee-row">
                      ...
                      <span style={{ color: highImpact ? '#f87171' : '#22c55e88' }}>
                        {priceImpactPct.toFixed(2)}% {highImpact && '⚠ HIGH'}
                      </span>
                    </div>
            ...
            <div className="cc-modal-f">
              ...
              <button className="cc-btn-ok" style={{ background:'linear-gradient(135deg,#16a34a,#15803d)', color:'#fff' }}
                onClick={async () => {
                  ...
                  // ✅ FIX: Calculate real minOut with 2% slippage — not hardcoded 0
                  const pool = ammModal.pool;
                  ...
                  if (ammDir === 'buy' && pool.ethReserve > 0 && pool.creditReserve > 0) {
                    const inputWithFee = inputAmt * 0.997;
                    const expectedOut = (inputWithFee * pool.creditReserve) / (pool.ethReserve + inputWithFee);
                    minOut = Math.floor(expectedOut * 0.98); // 2% slippage tolerance
                  } else if (ammDir === 'sell' && pool.creditReserve > 0 && pool.ethReserve > 0) {
                    const inputWithFee = inputAmt * 0.997;
                    const expectedOut = (inputWithFee * pool.ethReserve) / (pool.creditReserve + inputWithFee);
                    minOut = expectedOut * 0.98;
                  }
