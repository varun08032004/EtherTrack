...
// Use global apiFetch which handles auto token refresh
...
const AdminDashboard = () => {
  ...
  const loadCredits = useCallback(async () => {
    setLoading(true);
    try { const d = await apiFetch(`/api/admin/credits?status=${creditFilter}`); setCredits(d.credits); }
    catch {} finally { setLoading(false); }
  }, [creditFilter]);
    ...
    <div style={S.page}>
      ...
      <div style={S.main}>
        ...
        {tab === 'overview' && (
          <div style={{ ...S.section, marginTop:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10 }}>
              {[
                { label:'Backend API',       status:'online',  note:'localhost:5000' },
                ...
                { label:'Email (Resend)',      status:'online',  note:'help.ethertrack' },
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:20 }}>
              {['pending','approved','rejected'].map(s => (
                <button key={s} style={{ ...S.filterBtn, ...(kycFilter===s ? S.filterBtnActive : {}) }}
                ...
                {kyc.map(k => (
                  <div key={k.id} style={S.tableRow}>
                    ...
                    <div style={{ ...S.td, display:'flex', gap:6 }}>
                      <button style={S.viewBtn} onClick={() => setModal({ type:'kyc_detail', data: k })}>
                        DETAILS
                      </button>
                      {k.status === 'pending' && (
                        <>
                          <button style={S.approveBtn}
                            onClick={() => setModal({ type:'kyc_approve', data: k })}>
                            ✓
                          </button>
                          <button style={S.rejectBtn}
                            onClick={() => setModal({ type:'kyc_reject', data: k })}>
                            ✕
                          </button>
                        </>
                      )}
        ...
        {tab === 'credits' && (
          <div>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:20 }}>
              {['pending','approved','rejected'].map(s => (
                <button key={s} style={{ ...S.filterBtn, ...(creditFilter===s ? S.filterBtnActive : {}) }}
            ...
            {loading ? <div style={S.loading}>Loading...</div> : (
              <div style={S.table}>
                <div style={{ ...S.tableHead, gridTemplateColumns:'repeat(7,1fr)' }}>
                  {['USER', 'REGISTRY', 'SERIAL NO.', 'QUANTITY', 'VINTAGE', 'PROOF', 'ACTIONS'].map(h => (
                    <div key={h} style={S.th}>{h}</div>
                  ))}
                </div>
                {credits.length === 0 && <div style={S.empty}>No {creditFilter} credit listings</div>}
                {credits.map(c => (
                  <div key={c.id} style={{ ...S.tableRow, gridTemplateColumns:'repeat(7,1fr)' }}>
                    <div style={S.td}>
                      <div style={{ color:'#f0fdf4', fontSize:11 }}>{c.full_name}</div>
                      <div style={{ color:'#f59e0bcc', fontSize:9 }}>{c.email}</div>
                    </div>
                    <div style={{ ...S.td, fontSize:10, color:'#f0fdf4' }}>{c.registry_name || '—'}</div>
                    ...
                    <div style={{ ...S.td, display:'flex', gap:6 }}>
                      ...
                      {c.admin_status === 'pending' && (
                        <>
                          <button style={S.approveBtn}
                            onClick={() => setModal({ type:'credit_approve', data: c })}>✓</button>
                          <button style={S.rejectBtn}
                            onClick={() => setModal({ type:'credit_reject', data: c })}>✕</button>
                        </>
                      )}
                    </div>
                  </div>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
              ...
              {['','frozen','verified','pending'].map(s => (
                <button key={s} style={{ ...S.filterBtn, ...(userFilter===s ? S.filterBtnActive : {}) }}
              ...
              <div style={S.table}>
                ...
                {users.map(u => (
                  <div key={u.id} style={{ ...S.tableRow, ...(u.frozen ? { background:'#1a0a0a22' } : {}) }}>
                    ...
                    <div style={{ ...S.td, display:'flex', gap:6 }}>
                      <button style={S.viewBtn} onClick={() => setModal({ type:'user_detail', data: u })}>
                        VIEW
                      </button>
                      {!u.frozen
                ...
                {disputes.map(d => (
                  <div key={d.id} style={{ ...S.tableRow, gridTemplateColumns:'repeat(5,1fr)' }}>
                    ...
                    <div style={S.td}>
                      {d.status === 'open' && (
                        <button style={S.approveBtn}
                          onClick={() => setModal({ type:'resolve_dispute', data: d })}>
                          RESOLVE
                        </button>
                      )}
                      {d.status === 'resolved' && (
      ...
      {modal?.type === 'credit_detail' && (
        <Modal title="Credit Listing Details" onClose={() => setModal(null)}>
          {[
            ...
            ['Email',          modal.data.email],
            ['Registry Name',  modal.data.registry_name || '—'],
          ...
          ].filter(([k]) => !['Vintage Year'].includes(k) || true).map(([k, v]) => (
            <div key={k} style={M.row}>
              <span style={M.key}>{k}</span>
              <span style={M.val}>
      ...
      {modal?.type === 'credit_approve' && (
        <Modal title="Approve Credit Listing" onClose={() => { setModal(null); setReason(''); }}>
          <div style={M.confirmText}>
            ...
            Serial: <span style={{ color:'#60a5fa' }}>{modal.data.registry_serial || '—'}</span><br/>
            Registry: <span style={{ color:'#f59e0bdd' }}>{modal.data.registry_name || '—'}</span>
          </div>
          ...
          <button style={M.approveBtn} onClick={() => creditAction(modal.data.id, 'approve')}>
            CONFIRM APPROVE
          </button>
        </Modal>
      )}
      ...
      {modal?.type === 'freeze' && (
        <Modal title="Freeze Account" onClose={() => { setModal(null); setReason(''); }}>
          ...
          <textarea style={M.textarea} value={reason} onChange={e => setReason(e.target.value)}
            placeholder="Reason for freezing e.g. Suspicious trading activity..."/>
      ...
      {modal?.type === 'unfreeze' && (
        <Modal title="Unfreeze Account" onClose={() => { setModal(null); setReason(''); }}>
          ...
          <button style={M.approveBtn} onClick={() => freezeAction(modal.data.id, 'unfreeze')}>
            CONFIRM UNFREEZE
          </button>
        </Modal>
      )}
